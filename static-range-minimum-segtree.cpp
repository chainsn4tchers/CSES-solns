...
struct segtree {
    size_t size;
    vector<ll> tree;
    int64_t identity = __INT64_MAX__;
    ...
    ll rangeMin(int lo, int hi, int idx, int l, int r)
    {
    	  ...
    	  if (lo == l && hi == r) return tree[idx];
    	  int mid = lo + ((hi - lo) / 2);
    	  return min(rangeMin(lo, mid, (2 * idx) + 1, l, min(r, mid)), rangeMin(mid + 1, hi, (2 * idx) + 2, max(l, mid + 1), r));
    }
};
...
void tabsize_3(int tc = 1)
{
	...
	while (q--) {
		get2(l, r);
		log(tree.rangeMin(0, n - 1, 0, l - 1, r - 1));
	}
}
